...
def run_retrieval(conn, query):
    """Run P9+ retrieval and parse output."""
    try:
        from p9_retrieve import p9_retrieve
        f = io.StringIO()
        with redirect_stdout(f):
            p9_retrieve(conn, query)
    ...
    for exp in exp_scs:
        ...
        for ret in ret_scs:
            ...
            if exp_lower in ret_lower or ret_lower in exp_lower:
                ...
                ret_words = set(ret_lower.split())
                overlap = len(exp_words & ret_words)
                if overlap >= 2 or (overlap >= 1 and len(exp_words) <= 2):
                    sc_hits += 1
                    break
    sc_recall = sc_hits / len(exp_scs) if exp_scs else 1.0

    # Anchor recall: did expected anchors appear?
    all_ancs = []
