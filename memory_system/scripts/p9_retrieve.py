...
# Try neural embedding engine (singleton pattern)
_emb_singleton = None
try:
    from embedding_engine import EmbeddingSearch as _EmbeddingSearch, INDEX_FILE as _EMB_INDEX
    HAS_NEURAL = _EMB_INDEX.exists()
except ImportError:
    HAS_NEURAL = False

def _get_emb_searcher():
    """Singleton — load embedding index once, reuse across all retrievals."""
    global _emb_singleton
    if _emb_singleton is None or not _emb_singleton.ready:
        try:
            _emb_singleton = _EmbeddingSearch()
        except Exception:
            return None
...
SYNONYMS = {
    ...
    "memory": ["recall", "retrieve", "remember", "knowledge", "pmis"],
    "retrieve": ["recall", "fetch", "find", "search", "lookup", "memory"],
    "system": ["architecture", "design", "framework", "engine", "platform"],
    "case": ["study", "example", "reference", "story"],
    "outreach": ["cold", "email", "campaign", "prospecting"],
    "carousel": ["slide", "deck", "presentation"],
    "qualify": ["qualification", "evaluate", "assess", "score"],
    "pitch": ["propose", "present", "sell", "demo"],
    "template": ["structure", "format", "layout", "framework"],
}
...
def _expand_synonyms(tokens):
    """C1: Expand query tokens with domain synonyms. Also returns boost map."""
    expanded = set(tokens)
    for t in tokens:
        if t in SYNONYMS:
...
def _get_weights(n_anchors):
    # Neural embeddings: strong but should not dominate keyword matching
    # Keywords handle exact matches; neural handles semantic gaps
    if HAS_NEURAL:
        if n_anchors < 500:
            return 0.30, 0.35, 0.35  # tag, bm25, neural_vec (balanced)
        elif n_anchors < 5000:
            return 0.20, 0.40, 0.40
        else:
            return 0.15, 0.45, 0.40
    else:
        # TF-IDF fallback: keyword-heavy
        if n_anchors < 500:
            return 0.40, 0.35, 0.25  # tag, bm25, tfidf_vec
        elif n_anchors < 5000:
...
def p9_retrieve(conn, query, top_k=10):
    """
    ...
    # P12 OPT: Cache evidence weights — compute once, reuse everywhere
    _ew_cache = {}
    def cached_ew(anchor_id):
        if anchor_id not in _ew_cache:
            _ew_cache[anchor_id] = mem.compute_evidence_weight(conn, anchor_id)
        return _ew_cache[anchor_id]
    ...
    # Use neural embeddings if available, fall back to TF-IDF
    if HAS_NEURAL:
        _emb_searcher = _get_emb_searcher()  # P12 OPT: singleton
        _neural_mode = _emb_searcher.ready if _emb_searcher else False
        vectors = VectorEngine()  # dummy, won't be used
    else:
        _emb_searcher = None
    ...
    # ── Query expansion: synonyms + stems + abbreviations ──
    raw_tokens = _tok(query)
    expanded_tokens = _expand_synonyms(raw_tokens)
    expanded_tokens = _expand_abbreviations(list(expanded_tokens))
    expanded_tokens = _stem_tokens(list(expanded_tokens))
    # Build expanded query string for vector search
    expanded = query + " " + " ".join(expanded_tokens - set(raw_tokens))

    query_tokens = list(expanded_tokens)
    query_tags = set(query_tokens)

    # Also add raw tokens (always present)
    for w in raw_tokens:
        query_tags.add(w)
    ...
    for i, nid in enumerate(node_order):
        ...
        if node["type"] == "anchor":
            n_weight = cached_ew(nid)
        bm25_sc[sc] += s * t_weight * n_weight

    # ── Vector search (neural or TF-IDF) ──
    vec_sc = defaultdict(float)
    if _neural_mode or vectors.active:
        if _neural_mode and _emb_searcher:
            # Neural engine returns (node_id, score, type) triples
            neural_results = _emb_searcher.search(expanded, top_k=30)
            for nid, sim, _ntype in neural_results:
                if sim < 0.1:
                    continue
                sc = node_sc_map.get(nid)
                if not sc:
            ...
            # TF-IDF fallback: returns flat array aligned with node_order
    ...
    # ── SC-title direct match bonus ──
    # If raw query tokens appear in the SC title, that SC gets a significant boost
    raw_lower = set(w.lower() for w in raw_tokens)
    for sid in list(merged.keys()):
        sc = nodes_by_id.get(sid)
        if not sc:
            continue
        sc_title_tokens = set(w.lower().strip(".,;:!?()[]{}\"'/-") for w in sc["title"].split()
                              if len(w) > 2)
        overlap = raw_lower & sc_title_tokens
        if overlap:
            # Proportional boost: more query words in title = bigger boost
            title_match_ratio = len(overlap) / max(len(raw_lower), 1)
            # Strong multiplicative boost (1.0 to 3.0x based on overlap)
            merged[sid] *= (1.0 + 2.0 * title_match_ratio)

    # ── Per-SC node count normalization ──
    # Prevent large SCs from dominating just by having more nodes
    for sid in merged:
        n_nodes = len(children.get(sid, []))
        for ctx_id in children.get(sid, []):
            n_nodes += len(children.get(ctx_id, []))
        if n_nodes > 5:
            # Soft normalization: diminishing returns after 5 nodes
            merged[sid] *= (5.0 / n_nodes) ** 0.3
...
def p9_retrieve_parameterized(conn, query, params=None, session=None, top_k=3):
    ...
    # Build texts (P11: include description for SCs and contexts)
    node_order = []
    node_sc_map = {}
    texts = []
    for n in all_nodes:
        ...
        node_sc_map[n["id"]] = sc_id
        t = n["title"]
        # Include description if available (critical for SC-level BM25 matching)
        if n.get("description") and len(n["description"]) > 5:
            t += " " + n["description"][:150]
        pid = parent_of.get(n["id"])
        if pid and pid in nodes_by_id:
            t = nodes_by_id[pid]["title"] + " " + t
            ...
            if sc_id2 and sc_id2 != pid and sc_id2 in nodes_by_id:
                t = nodes_by_id[sc_id2]["title"] + " " + t
        if n.get("content") and n["content"] != n["title"]:
            content_limit = int(100 * p_anchor_cw / 0.5)
