...
def cmd_retrieve(conn, query):
    """Retrieve memories with temporal awareness and winning-structure priority."""
...
def _record_retrieval(conn, context_pack):
    """Record which anchors were served in this retrieval for future scoring."""
    # Store as a lightweight retrieval event
    retrieval_file = TASKS_DIR / f"retrieval_{_id()}.json"
    anchor_ids = []
    for mem in context_pack.get("memories", []):
        for ctx in mem.get("contexts", []):
            for anc in ctx.get("anchors", []):
                # We don't have IDs in the output — this is tracked via task_anchors at store time
                pass
    # Retrieval tracking is handled when the next store/score references a task
    # The task_anchors table already records what was present
